...
void CalcDipole(double mi[3], double mj[3], double dissq, double dij[3], double F_0, double f_mag[3]) {
    ...
    // Note that the negative sign before dij resulting from the choice of direction for dij
    for (int d = 0; d < dim; d++) {
...
// "f" is split in "f_ste" (f steric, i.e., WCA) and "f_mag" (f magnetic, i.e., dipole) 
...
double CalcPairMag(Particle *p_i, Particle *p_j) {
    ...
    // of magnetic particle interactions after "rcut_dipol_part" micromagnets 
