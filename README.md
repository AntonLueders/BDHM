...
The program can be built for gcc 13.2 (through GitHub). It was heavily tested for gcc 6.3.0 and gcc 7.5.0. The data presented in [1] was mostly obtained through a binary compiled with icc.

To compile BDHM, the library GSL must be linked. However, to repeat the studies of [1], GSL is only used for random numbers to generate the random initial conditions. Replacing these random initial conditions, the program can easily be adjusted so that GSL is not needed.
